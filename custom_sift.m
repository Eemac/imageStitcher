function [final_slope, final_length, final_delta_x, final_delta_y, final_confidence] = custom_sift(left,right,radius,suppress)
...
%   Detailed explanation goes here
    ...
    [slopes, lengths, delta_x, delta_y, inliers] = filter_matches(left, x1, y1, x2, y2, 0.25, 0.25, suppress);
    final_slope = mean(slopes(inliers));
    final_length = mean(lengths(inliers));
    final_delta_x = mean(delta_x(inliers));
    final_delta_y = mean(delta_y(inliers));
